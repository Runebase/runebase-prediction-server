...
async function updateTx(tx, currentBlockCount) {
  ...
  if (tx.type === TX_TYPE.TRANSFER && tx.token === TOKEN.RUNEBASE && !tx.blockNum) {
    await updateRunebaseTransferTx(tx, currentBlockCount);
    return;
  }
...
/**
 * Updates a Runebase transfer tx. The confirmation method is not the same as an EVM tx confirmation.
 * @param {Transaction} tx Transaction to update.
 * @param {number} currentBlockCount Current block number.
 */
async function updateRunebaseTransferTx(tx, currentBlockCount) {
  try {
    ...
    getLogger().error(`updateRunebaseTransferTx: ${err.message}`);
    throw Error(`updateRunebaseTransferTx: ${err.message}`);
  }
}
