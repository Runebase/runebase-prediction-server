      ...
      if (rawLog._eventName === 'OracleResultVoted') {
        try {
          ...
          if (vote.token === 'RUNEBASE') {
            ...
            if (oracle.token === 'RUNEBASE') vote.type = VOTE_TYPE.RESULT_SET;
            else if (oracle.token === 'PRED') vote.type = VOTE_TYPE.VOTE;
          }
          ...
          switch (vote.token) {
            case TOKEN.RUNEBASE: {
              topic.runebaseAmount[vote.optionIdx] =
                new BigNumber(topic.runebaseAmount[vote.optionIdx]).plus(voteBn).toString(10);
            ...
            case TOKEN.PRED: {
              topic.predAmount[vote.optionIdx] =
                new BigNumber(topic.predAmount[vote.optionIdx]).plus(voteBn).toString(10);
              await DBHelper.updateObjectByQuery(
                db.Topics,
                { address: topic.address },
          ...
          // PRED. Setting the result creates an OracleResultVoted event and takes place in the COracle contract.
...
const updateCOraclesDoneResultSet = async (currentBlockTime) => {
  try {
    await db.Oracles.update(
      { resultSetEndTime: { $lt: currentBlockTime }, token: TOKEN.RUNEBASE, status: STATUS.WAITRESULT },
