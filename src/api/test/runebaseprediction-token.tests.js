...
const Mocks = require('./mock/runebaseprediction-token');
...
describe('RunebasePredictionToken', () => {
  ...
  describe('balanceOf()', () => {
    ...
    it('throws if owner is undefined', () => {
      expect(RunebasePredictionToken.balanceOf({ senderAddress: TestConfig.SENDER_ADDRESS })).to.be.rejectedWith(Error);
    });

    it('throws if senderAddress is undefined', () => {
      expect(RunebasePredictionToken.balanceOf({ owner: 'qKjn4fStBaAtwGiwueJf9qFxgpbAvf1xAy' })).to.be.rejectedWith(Error);
    });
  });
});
