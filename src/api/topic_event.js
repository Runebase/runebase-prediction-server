...
function getContract(contractAddress) {
  ...
  return new Contract(getRunebaseRPCAddress(), contractAddress, metadata.TopicEvent.abi);
}
...
const TopicEvent = {
  ...
  async totalRunebaseValue(args) {
    ...
    const contract = getContract(contractAddress);
    const res = await contract.call('totalRunebaseValue', {
      methodArgs: [],
      senderAddress,
    });
  ...
  async totalPredValue(args) {
    ...
    const contract = getContract(contractAddress);
    const res = await contract.call('totalPredValue', {
      methodArgs: [],
      senderAddress,
    });
    res[0] = Utils.hexToDecimalString(res[0]);
