...
// Gets the RUNEBASE and PRED balances for all ever used addresses
const getAddressBalances = async () => {
  ...
  // Get PRED balances of every address
  ...
  await new Promise(async (resolve) => {
    sequentialLoop(batches.length, async (loop) => {
      ...
      _.map(batches[loop.iteration()], async (address) => {
        promises.push(new Promise(async (getPredBalanceResolve) => {
          let predBalance = new BigNumber(0);

          // Get PRED balance
          try {
            const res = await RunebasePredictionToken.balanceOf({
              owner: address,
              senderAddress: address,
            });
            predBalance = res.balance;
          } catch (err) {
            getLogger().error(`getAddressBalances RunebasePredictionToken.balanceOf ${address}: ${err.message}`);
          }
...
module.exports = {
  ...
  winners: async (root, { filter, orderBy, limit, skip }, { db: { Votes } }) => {
    ...
    winnings = _.orderBy(winnings, [function (o) { return o.amount.runebase; }], ['desc']);
    return winnings;
  },
  ...
  leaderboardStats: async (root, { filter, orderBy, limit, skip }, { db: { Votes, Topics } }) => {
    ...
    let totalRunebase = new BigNumber(0);
    let totalPred = new BigNumber(0);
    result.reduce((acc, cur) => {
      ...
      if (cur.token === TOKEN.PRED) {
        totalPred = new BigNumber(totalPred).plus(curAmount);
      } else {
        totalRunebase = new BigNumber(totalRunebase).plus(curAmount);
      }
      return acc;
    }, {});
    return {
      ...
      participantsCount,
      totalRunebase: totalRunebase.toString(10),
      totalPred: totalPred.toString(10),
    };
  },
